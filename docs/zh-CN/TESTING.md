...
- `scripts/smoke/base.mjs` 创建基础 TanStack Start 应用，运行 `pnpm install`、`pnpm test`、`pnpm lint`、`pnpm typecheck`，启动 dev server，检查 `/`、`/health`、`/api/health`、`/api/v1/me`、trusted API CORS 和默认收紧的 CORS 行为，然后运行 `pnpm build`。
