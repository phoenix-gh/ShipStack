...
- `scripts/smoke/auth.mjs` 创建应用，安装 database 与 Better Auth 模块，重复安装两次 auth 模块，检查模块中文文档，验证 lint，生成 auth migrations，用 Wrangler 本地应用 migrations，启动 dev server，验证匿名 dashboard redirect，登录后检查认证态 `/api/v1/me`，运行浏览器注册、浏览器认证态 `/api/v1/me`、清除 session、重新登录和 dashboard 检查，然后运行同样的应用检查。
