...
This repository is now a local `v0.1.0` MVP release candidate with a monorepo scaffold, CLI, base TanStack Start + Cloudflare Workers template, D1 + Drizzle module, Better Auth module, Stripe billing module, R2 storage module, API keys recipe, OpenAPI recipe, API rate limit recipe, protected auth pages, generated-app smoke tests, auth browser smoke checks, and CI/release workflows. Run `pnpm release:audit` for the current gate status. The remaining external release gates are a real Cloudflare deployment pass, remote GitHub Actions confirmation, and npm publish workflow dry-run on the remote repository.
