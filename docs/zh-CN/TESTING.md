...
- `scripts/smoke/auth.mjs` 创建应用，安装 database 与 Better Auth 模块，重复安装两次 auth 模块，生成 auth migrations，用 Wrangler 本地应用 migrations，启动 dev server，验证匿名 dashboard redirect，登录后检查认证态 `/api/v1/me`，然后运行同样的应用检查。
...
2. 浏览器测试，覆盖注册、登录、退出和受保护 dashboard 行为。
3. Cloudflare 手动部署验证。
